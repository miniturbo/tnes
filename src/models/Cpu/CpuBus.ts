...
export class CpuBus {
  ...
  ppu: Ppu | null = null
  apu: Apu | null = null
  workRam: Ram | null = null
  controller1: Controller | null = null
  controller2: Controller | null = null
  dmaController: DmaController | null = null

  read(address: Uint16): Uint8 {
    if (address >= 0x0000 && address <= 0x1fff) {
      return this.workRam?.read(address & 0x07ff) ?? 0x00
    } else if (address >= 0x2000 && address <= 0x3fff) {
      return this.ppu?.readRegister(address & 0x2007) ?? 0x00
      ...
      return this.controller1?.read() ?? 0x00
    } else if (address === 0x4017) {
      return this.controller2?.read() ?? 0x00
    } else if (address >= 0x4018 && address <= 0x7fff) {
      // TODO
      return 0x00
    } else if (address >= 0x8000 && address <= 0xffff) {
      return this.rom?.readProgramRom(address) ?? 0x00
    } else {
